...
{
	float x = initial->pose.pose.position.x / grid->info.resolution;
	float y = initial->pose.pose.position.y / grid->info.resolution;
...
void Planner::setGoal(const geometry_msgs::PoseStamped::ConstPtr& end)
{
	...
	float x = end->pose.position.x / grid->info.resolution;
	float y = end->pose.position.y / grid->info.resolution;
...
void Planner::plan()
{
	...
	std::cout << "Is start point valid?" << validStart << std::endl;
	std::cout << "Is goal point valid?" << validGoal << std::endl;
	...
	if (validStart && validGoal) {
		...
		// retrieving goal position
		float x = goal.pose.position.x / grid->info.resolution;
		float y = goal.pose.position.y / grid->info.resolution;
		...
		x = start.pose.pose.position.x / grid->info.resolution;
		y = start.pose.pose.position.y / grid->info.resolution;
